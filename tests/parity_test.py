...
NoTabs = QMainWindow.DockOption.AnimatedDocks  # AllowTabbedDocks intentionally absent
...
def float_all(qt_win, qt_docks, l_win, l_docks):
...
def make_dock_all(setup: list):
    ...
    def action(qt_win, qt_docks, l_win, l_docks):
...
def run_scenario(scenario: dict, app: QApplication) -> list:
    ...
    for action in scenario["actions"]:
        action(qt_win, qt_docks, l_win, l_docks)
        app.processEvents()

    qt_s = qt_snap(qt_win, qt_docks)
    l_s  = l_snap(l_win, l_docks)
